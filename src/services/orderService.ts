...
export async function createOrderService(input: CreateOrderInput): Promise<CreateOrderResult> {
  ...
  if (requestId && resolvedPreReservedKotNumber != null) {
    ...
    if (redis) {
      ...
      redis.set(`kot:used:${tenantId}:${requestId}`, '1', 'EX', 120).catch(() => {});
    }
  }
...
export async function updateOrderItemsService(input: UpdateOrderItemsInput): Promise<UpdateOrderItemsResult> {
    ...
    async (tx) => {
      ...
      if (!isExtraTable) {
        updatedTable = await tx.table.update({
          ...
          data: {
            status: existing.status === OrderStatus.BILLING_REQUESTED ? TableStatus.BILLING_REQUESTED : TableStatus.PREPARING,
  ...
  if (requestId && resolvedPreReservedKotNumber != null) {
    ...
    if (redis) {
      ...
      redis.set(`kot:used:${existing.restaurantId}:${requestId}`, '1', 'EX', 120).catch(() => {});
    }
  }
...
export async function cancelOrderItemsService(input: CancelOrderItemsInput): Promise<CancelOrderItemsResult> {
  ...
  if (cancelledItemsMeta.length > 0) {
    ...
    // Group cancelled items by printer name (same grouping key as create-KOT).
    // When printerName is not resolved, fall back to effectiveTarget for legacy
    // routing — mirroring how regular KOT splits (BAR_PRINTER or LIQUOR → bar,
    // else → kitchen).
    ...
    for (const item of cancelledItemsMeta) {
      const effectiveTarget = item.printerTarget
        || (item.menuType === 'LIQUOR' ? 'BAR_PRINTER' : 'KOT_PRINTER');
      // Same key as create-KOT: group by printerName, fallback to effectiveTarget
      const key = item.printerName || `__${effectiveTarget}__`;
...
export async function settleOrderService(input: SettleOrderInput): Promise<SettleOrderResult> {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // -- Inventory deduction (bar + kitchen) -------------------------------
    // Extracted into deductInventoryForOrder() so it can be shared between
    // settleOrderService (cloud-direct settle) and upsertTransaction (edge sync).
    ...
    barDeductionErrors.push(...deductionResult.barDeductionErrors);
    kitchenDeductionErrors.push(...deductionResult.kitchenDeductionErrors);
    missingRecipeItems.push(...deductionResult.missingRecipeItems);
